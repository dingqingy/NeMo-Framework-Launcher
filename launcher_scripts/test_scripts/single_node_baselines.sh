...
WARMUP_FLUSH_OVERLAP=false

for M in 16; do 
# for M in 4 12 16 24; do 
    if [[ $M -eq 12 ]]
    then
        VP=8 # accomadate for comparing bfs N12M12
    else
        VP=11 # normal GPT3-20B
    fi
    NUM_LAYERS=$((PP*VP))
    GBS=$((MBS*M*DP))
    for P2P_OVERLAP in true false; do
        RUN_PREFIX=${MCORE_BRANCH}_N${N}M${M}
